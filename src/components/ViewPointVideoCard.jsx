const ViewPointVideoCard = ({ data }) => {
  ...
  return (
    ...
    >
      <div className="mb-0 flex w-full max-w-[248px] flex-col gap-1 rounded bg-white p-3 text-black">
        <h1 className="order-2 text-xs font-bold">{author}</h1>
        <p className="order-1 max-w-[30ch] text-xs font-normal">{content}</p>
        <a href={videoLink} className="btn btn-big btn-icon order-3 mt-1">
