...
const MainMenu = ({ relativeClasses }) => {
  ...
  return (
    <nav className={`${relativeClasses} group flex w-fit`}>
      ...
      <div
        className={`${showMenu ? "scale-100 opacity-100" : "scale-0 opacity-0"} fixed inset-0 z-10 h-screen w-screen bg-black/5 backdrop-blur-2xl transition-transform duration-100 peer-focus-within:scale-100 peer-focus-within:opacity-100 peer-focus-visible:scale-100 peer-focus-visible:opacity-100 lg:hidden`}
      ></div>
    </nav>
  );
};
